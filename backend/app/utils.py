...
# New dependencies for budget tracking
from budget_service import budget_service
from usage_worker import enqueue_request_log

__name__ = "hanseware.fast-openai-api-proxy.utils"

RESERVED_ATTRS: List[str] = [
    "args", "created", "exc_info", "exc_text", "filename", "levelno",
    "lineno", "module", "msecs", "message", "msg", "process", "processName",
...
async def _resolve_budget(request: Request, model_data: dict, usage_value: Optional[float], elapsed: float, body: dict, is_error: bool):
    payer_entity = getattr(request.state, "payer_entity", None)
    if not payer_entity:
        return
        
    if getattr(request.state, "budget_resolved", False):
        return
    request.state.budget_resolved = True
        
    reserved_credits = getattr(request.state, "reserved_credits", 0.0)
    m_type = model_data.get("type", "llm")
    price_per_unit = model_data.get("price_per_unit", 0.0)
    
    final_usage = 0.0
    usage_unit = "tokens"
    
    if not is_error:
        if m_type in ("llm", "embedding"):
            final_usage = usage_value or 0.0
            usage_unit = "tokens"
        elif m_type == "image-gen":
            if body and "n" in body:
                try:
                    final_usage = float(body["n"])
                except:
                    final_usage = 1.0
            else:
                final_usage = 1.0
            usage_unit = "images"
        elif m_type in ("stt", "tts", "realtime"):
            final_usage = usage_value if usage_value is not None else elapsed
            usage_unit = "seconds"
        else:
            final_usage = usage_value or 1.0
            usage_unit = "units"
            
    cost = final_usage * price_per_unit
    
    # Resolve in-memory budget
    await budget_service.resolve_budget(payer_entity[0], payer_entity[1], m_type, reserved_credits, cost)
    
    # Add to request logs queue
    api_key_ctx = getattr(request.state, "api_key", None)
    api_key_id = api_key_ctx["id"] if api_key_ctx else None
...
async def handle_request(request: Request, api_path: str):
    ...
    model_data = getattr(request.state, "model_data", None) or models.get_model_data(model, api_path)
    if not model_data:
    ...
    if stream and model_data.get("type", "llm") in ("llm", "embedding"):
        ...
        if model_data.get('fallback_model_name'):
            ...
            if fallback_data:
                ...
                try:
                    ...
                    await client.aclose()
                    await _resolve_budget(request, model_data, None, time.time() - t_start, body, True)
    ...
    if stream:
        ...
        if is_responses_api:
            ...
            async def wrapped_translated_stream():
                usage_value = None
                try:
                                ...
                                if line.startswith("data: ") and line.strip() != "data: [DONE]":
                                    try:
                                        ...
                                        if "usage" in j and j["usage"]:
                                            usage_value = j["usage"].get("total_tokens")
                                    except: pass
                        yield chunk
                finally:
                    await _resolve_budget(request, model_data, usage_value, time.time() - t_start, body, False)
                    await response.aclose()
                    await client.aclose()
                    
            return StreamingResponse(
                wrapped_translated_stream(),
        ...
        usage_value = None
        if not is_error and "application/json" in response.headers.get("Content-Type", ""):
            try:
                resp_json = response.json()
                if "usage" in resp_json:
                    usage_value = resp_json["usage"].get("total_tokens")
                elif "model_extra" in resp_json and "usage" in resp_json["model_extra"]:
                    usage_value = resp_json["model_extra"]["usage"]
            except Exception: pass
            
        await _resolve_budget(request, model_data, usage_value, time.time() - t_start, body, is_error)
        
        if is_responses_api and not is_error:
            try:
...
async def handle_subresource_request(
    ...
    if stream:
        ...
        async def stream_generator():
            try:
                ...
                await _resolve_budget(request, model_data, None, time.time() - t_start, body, False)
                await upstream_response.aclose()
                await client.aclose()
    ...
    is_error = response.status_code >= 400
    await _resolve_budget(request, model_data, None, time.time() - t_start, body, is_error)
...
async def handle_file_upload(
    ...
    if stream_response:
        ...
        async def stream_generator():
            try:
                ...
                await _resolve_budget(request, model_data, None, time.time() - t_start, data, False)
                await client.aclose()
    ...
    async with httpx.AsyncClient(timeout=custom_timeout) as client:
        try:
            ...
            await _resolve_budget(request, model_data, None, time.time() - t_start, data, True)
            raise HTTPException(status_code=502, detail=f"Upstream request failed: {exc}") from exc

    is_error = response.status_code >= 400
    await _resolve_budget(request, model_data, None, time.time() - t_start, data, is_error)
